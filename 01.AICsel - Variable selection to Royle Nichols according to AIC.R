# ============================================================
# Backward AIC selection for unmarked::occuRN()
# Uses unique names so it will not clash with your other scripts.
#
# - occuRN expects a double-RHS formula:  ~ det_covs ~ state_covs
...
rnAIC_get_aic <- function(fit) {
  ...
  # 2) Compute from optim output (most robust for unmarkedFit objects)
  ...
  stop("rnAIC_get_aic: Could not extract or compute AIC for this occuRN fit.")
}

  

backward_occuRN_AIC_rnAIC <- function(det_formula,
                                      state_formula,
                                      data,
  ...
  rnAIC_fit_once <- function(df, sf) {
    form <- rnAIC_as_double_rhs(df, sf)
    unmarked::occuRN(formula = form, data = data, ...)
  }
  
  det_terms   <- rnAIC_get_terms(det_formula)
  state_terms <- rnAIC_get_terms(state_formula)
  ...
  cur_fit <- rnAIC_fit_once(
    rnAIC_make_one_sided(cur_det_terms),
    rnAIC_make_one_sided(cur_state_terms)
  )
  cur_aic <- rnAIC_get_aic(cur_fit)
  ...
  for (step in seq_len(max_steps)) {
    ...
    for (i in seq_len(nrow(cand))) {
      ...
      fit_i <- tryCatch(
        rnAIC_fit_once(
          rnAIC_make_one_sided(new_det_terms),
          rnAIC_make_one_sided(new_state_terms)
        ),
        error = function(e) NULL
      )
      if (is.null(fit_i)) next
      
      aic_i <- tryCatch(rnAIC_get_aic(fit_i), error = function(e) Inf)
  ...
  list(
    best_model = cur_fit,
    best_det_formula   = rnAIC_make_one_sided(cur_det_terms),
    best_state_formula = rnAIC_make_one_sided(cur_state_terms),
    trace = trace,
    models = models
  )
}
...
# Example call
# ============================================================
# sel_rn_aic <- backward_occuRN_AIC_rnAIC(
...
# )
#
# summary(sel_rn_aic$best_model)
# sel_rn_aic$best_det_formula
# sel_rn_aic$trace
